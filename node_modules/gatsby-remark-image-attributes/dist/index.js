...
const _options = {
  styleAttributes: ['width', 'height', 'margin-left', 'margin-right', 'margin-top', 'margin-bottom', 'float'],
...
const applyOptions = ({ styleAttributes, dataAttributes }, reporter) => {
  if (styleAttributes) {
    if (Array.isArray(styleAttributes)) {
      _options.styleAttributes = uniq(_options.styleAttributes.concat(styleAttributes.filter(isString)));
    } else {
      reporter.warn(logMsg`Option styleAttributes must be an Array of strings.`);
    }
  }
...
const createStyle = styleAttributes => Object.keys(styleAttributes).reduce((style, key) => `${style}${key}: ${styleAttributes[key]};`, '');

const createDataAttributes = dataAttributes => Object.keys(dataAttributes).reduce((data, key) => `${data} data-${key}="${dataAttributes[key]}"`, '');

const createImgMarkup = ({ attributes, url, alt }) => {
  const { styleAttributes, dataAttributes } = categorizeAttributes(attributes);
  return `<img src="${url}" style="${createStyle(styleAttributes) || 'width: 100%;'}" alt="${alt}" ${createDataAttributes(dataAttributes)} />`;
};

const wrapImgMarkup = ({ attributes, value }) => {
  const { styleAttributes, dataAttributes } = categorizeAttributes(attributes);
  return `<span style="display:block; ${createStyle(styleAttributes)}">${value.replace(/<img[^>]*/, `$& ${createDataAttributes(dataAttributes)}`)}</span>`;
};
...
module.exports = ({ markdownAST, reporter }, options) => {
  ...
  return Promise.all([new Promise(resolve => resolve(images.map(imageNode => {
    imageNode.type = 'html';
    imageNode.value = createImgMarkup(imageNode);
    return imageNode;
  }))), new Promise(resolve => resolve(gatsbyRemarkImagesHtml.map(imageNode => {
